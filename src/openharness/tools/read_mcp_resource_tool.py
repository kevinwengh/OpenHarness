...
class ReadMcpResourceTool(BaseTool):
    ...
    def __init__(self, manager: McpClientManager) -> None:
        """Initialize ``ReadMcpResourceTool`` and bind its runtime dependencies.

        Integration: Exposed through ``ReadMcpResourceTool``.
    ...
    def is_read_only(self, arguments: ReadMcpResourceToolInput) -> bool:
        """Classify whether this ``ReadMcpResourceTool`` invocation can mutate state.
    ...
    async def execute(self, arguments: ReadMcpResourceToolInput, context: ToolExecutionContext) -> ToolResult:
        """Execute one model-requested ``ReadMcpResourceTool`` invocation.

        Integration: Exposed through ``ReadMcpResourceTool`` and collaborates with
        ``ToolResult``, ``_manager.read_resource``.
