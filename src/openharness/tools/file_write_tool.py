...
class FileWriteTool(BaseTool):
    ...
    async def execute(
        ...
        """Execute one model-requested ``FileWriteTool`` invocation.

        Integration: Exposed through ``FileWriteTool`` and collaborates with ``_resolve_path``,
        ``is_docker_sandbox_active``, ``path.write_text``.
