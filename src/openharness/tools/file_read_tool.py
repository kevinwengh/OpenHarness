...
class FileReadTool(BaseTool):
    ...
    def is_read_only(self, arguments: FileReadToolInput) -> bool:
        """Classify whether this ``FileReadTool`` invocation can mutate state.

        Integration: Exposed through ``FileReadTool``.
    ...
    async def execute(
        ...
        """Execute one model-requested ``FileReadTool`` invocation.

        Integration: Exposed through ``FileReadTool`` and collaborates with ``_resolve_path``,
        ``is_docker_sandbox_active``, ``path.is_dir``.
