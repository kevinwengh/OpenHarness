...
class TeamDeleteTool(BaseTool):
    ...
    async def execute(self, arguments: TeamDeleteToolInput, context: ToolExecutionContext) -> ToolResult:
        """Execute one model-requested ``TeamDeleteTool`` invocation.

        Integration: Exposed through ``TeamDeleteTool`` and collaborates with ``ToolResult``,
        ``delete_team``, ``get_team_registry``.
